#!/usr/bin/env python3
"""测试 DROID 策略 PyTorch 模型推理的完整示例代码。

此脚本演示如何：
1. 加载预训练的 PyTorch DROID 策略模型
2. 创建模拟输入数据
3. 运行推理并获取动作输出
...
# PyTorch 模型路径
PYTORCH_CHECKPOINT_DIR = os.path.expanduser("~/.cache/openpi/openpi-assets/checkpoints/pi05_droid_pytorch")
...
def load_policy():
    ...
    if not os.path.exists(PYTORCH_CHECKPOINT_DIR):
        ...
        print("    --checkpoint_dir ~/.cache/openpi/openpi-assets/checkpoints/pi05_droid \\")
        print("    --output_path ~/.cache/openpi/openpi-assets/checkpoints/pi05_droid_pytorch \\")
        print("    --config_name pi05_droid \\")
